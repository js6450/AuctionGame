...
def determinebid(itemsinauction, winnerarray, winneramount, numberbidders, players, artists, standings, rd):
    ...
    for i in range(len(sorted_freq)):
        addon = standings[mybidderid][sorted_freq[i][0]]
        points[sorted_freq[i][0]] = 4 - i + addon
    ...
    bidval = random.randint(2, 4)
    average_price = 0
    max_price = 0
    r_money = standings[mybidderid]["money"]
    if(r_money <= 34):
        bidval = 0
	
    print("Round: " + str(rd + 1) + " with item " + itemsinauction[rd] + " for sale")
    print(sorted_points)
    print("Money left: " + str(r_money))
    print("Aiming to purchase: " + biditem)

    if(itemsinauction[rd] == biditem):
        if(rd == 0):
            bidval = 36
        elif(standings[mybidderid][biditem] == 2):
            bidval = r_money
        else:
		average_price = float(sum(winneramount)/len(winneramount))
		addon = 5 * (standings[mybidderid][biditem])
		if(max(winneramount) < 15):
			bidval = min(r_money, random.randint(15, 20) + addon)
		elif(max(winneramount) < 30 and max(winneramount) >= 15):
			max_price = max(winneramount) + random.randint(0,5)
			bidval = min(r_money, random.randint(int(average_price), max_price) + addon)
		else:
			max_price = 34
			bidval = min(r_money, random.randint(max_price - random.randint(1, 6), max_price) + addon)
